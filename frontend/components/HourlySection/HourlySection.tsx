...
import {
  MapPin, Clock, BriefcaseBusiness, Sparkles,
  Heart, Cake,
} from 'lucide-react';
import SectionHeading from '@/components/SectionHeading/SectionHeading';
import styles from './HourlySection.module.scss';
...
export default function HourlySection() {
  ...
  return (
    <section id="hourly" className={styles.section}>
      <div className={styles.container}>
        ...
        <div className={styles.features}>
          <div className={styles.feature}>
            ...
            <h3>{ru ? 'Гибкие маршруты' : 'Flexible routes'}</h3>
            <p>{ru ? 'Планируйте маршрут по своему усмотрению — несколько остановок без доплат за ожидание' : 'Plan your route freely — multiple stops with waiting included'}</p>
          </div>
          <div className={styles.feature}>
            <Clock className={styles.featureIcon} />
            <h3>{ru ? 'Водитель с вами' : 'Driver stays with you'}</h3>
            <p>{ru ? 'Автомобиль и водитель в вашем распоряжении всё время аренды' : 'Your car and chauffeur are at your service for the full rental'}</p>
          </div>
          <div className={styles.feature}>
            <Sparkles className={styles.featureIcon} />
            <h3>{ru ? 'Для любого повода' : 'For any occasion'}</h3>
            <p>{ru ? 'Свадьбы, юбилеи, выпускные и торжественные мероприятия' : 'Weddings, anniversaries, graduations and celebrations'}</p>
          </div>
          <div className={styles.feature}>
            <BriefcaseBusiness className={styles.featureIcon} />
            <h3>{ru ? 'Деловой формат' : 'Business ready'}</h3>
            <p>{ru ? 'Встречи, переговоры и поездки с несколькими остановками' : 'Meetings, negotiations and multi-stop itineraries'}</p>
          </div>
        </div>
        ...
        <div className={styles.notice}>
          <p>
            {ru
              ? 'Минимальная аренда — 3 часа. Стоимость рассчитывается по тарифу выбранного автомобиля и фиксируется при бронировании. Маршрут и количество остановок согласуются с менеджером после оформления заявки.'
              : 'Minimum rental is 3 hours. Price is based on the selected vehicle rate and locked at booking. Route and number of stops are agreed with our manager after reservation.'}
          </p>
        </div>

        <div className={styles.ctaRow}>
          <Link href={`/${locale}/booking?type=hourly`} className={styles.ctaBtn}>
            {ru ? 'Забронировать аренду' : 'Book rental'}
          </Link>
        </div>
      </div>
    </section>
